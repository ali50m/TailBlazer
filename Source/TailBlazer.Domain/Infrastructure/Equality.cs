...
public static class Equality
{
    public static IEqualityComparer<T> CompareOn<T, TValue>(Func<T, TValue> valueSelector)
    {
        return new GenericEqualityComparer<T>(
            (t1, t2) => EqualityComparer<TValue>.Default.Equals(valueSelector(t1), valueSelector(t2)),
            t => EqualityComparer<TValue>.Default.GetHashCode(valueSelector(t)));
    }

    public static IEqualityComparer<T> AndOn<T, TValue>(this IEqualityComparer<T> rootComparer,
        Func<T, TValue> valueSelector)
    {
        return new GenericEqualityComparer<T>(rootComparer,
            (t1, t2) => EqualityComparer<TValue>.Default.Equals(valueSelector(t1), valueSelector(t2)),
            t => EqualityComparer<TValue>.Default.GetHashCode(valueSelector(t)));
    }
...
public class GenericEqualityComparer<T> :IEqualityComparer<T>
{
    ...
    public GenericEqualityComparer(IEqualityComparer<T> rootComparer, Func<T, T, bool> compareFunction,
