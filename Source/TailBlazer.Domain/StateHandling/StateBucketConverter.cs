...
public class StateBucketConverter :IConverter<StateBucket[]>
{
    public StateBucket[] Convert(State state)
    {
        ...
        var files = root.Elements(Structure.Bucket)
            .Select(element =>
            {
                ...
                return new StateBucket(type, id, new State(stateVersion, stateValue),
    ...
    public State Convert(StateBucket[] buckets)
    {
        ...
        {
            var itemState = new XElement(new XElement(Structure.InnerState,
                new XAttribute(Structure.Version, bucket.State.Version)));
