...
public class IndexerFixture
{
    ...
    public void CanReadIndiciesBack_SmallFile()
    {
        ...
        file.Append(Enumerable.Range(1, 100).Select(i => $"This is line number {i.ToString("00000000")}")
            .ToArray());
...

        {
            ...
            var headExpected = Enumerable.Range(1, 10)
            ...
            var tailExpected = Enumerable.Range(91, 10)
                .Select(i => $"This is line number {i.ToString("00000000")}").ToArray();
            ...
            var midExpected = Enumerable.Range(21, 10)
                .Select(i => $"This is line number {i.ToString("00000000")}").ToArray();
    ...
    public void CanReadIndiciesBack_LargeFile()
    {
        ...
        using var file = new TestFile();
        file.Append(Enumerable.Range(1, 10000).Select(i => $"This is line number {i.ToString("00000000")}")
            .ToArray());
...

        {
            ...
            var tail = result.ReadLines(new ScrollRequest(10));
            var tailText = tail.Select(l => l.Text).ToArray();
            var tailExpected = Enumerable.Range(9991, 10)
                .Select(i => $"This is line number {i.ToString("00000000")}").ToArray();
            tailText.Should().BeEquivalentTo(tailExpected);

            var mid = result.ReadLines(new ScrollRequest(10, 100));
            var midText = mid.Select(l => l.Text).ToArray();
            var midExpected = Enumerable.Range(101, 10)
                .Select(i => $"This is line number {i.ToString("00000000")}").ToArray();
            midText.Should().BeEquivalentTo(midExpected);
        }
    }
    ...
    public void WillAutoTail()
    {
        ...
        using(indexer.Result.Subscribe(indicies => result = indicies))
        {
            ...
            file.Append(Enumerable.Range(10001, 10)
                .Select(i => $"This is line number {i.ToString("00000000")}").ToArray());
