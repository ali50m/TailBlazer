...
public class InlineViewer :AbstractNotifyPropertyChanged, ILinesVisualisation
{
        ...
        [NotNull] ILineMatches lineMatches)
    {
        ...
        var scrollSelected = selectedChanged
            .CombineLatest(lineProvider, pageSize,
                (proxy, lp, pge) => proxy == null ? new ScrollRequest(pge, 0) : new ScrollRequest(pge, proxy.Start))
        ...
        var proxyFactory = new LineProxyFactory(textFormatter, lineMatches, horizonalScrollArgs.DistinctUntilChanged(),
            themeProvider);
    ...
    void IScrollReceiver.ScrollBoundsChanged(ScrollBoundsArgs boundsArgs)
    {
        ...
        if(!_isSettingScrollPosition)
            _userScrollRequested.OnNext(
                new ScrollRequest(ScrollReason.User, boundsArgs.PageSize, boundsArgs.FirstIndex));
