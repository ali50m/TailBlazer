    ...
    IPageProvider, ISelectedAware
{
        ...
        [NotNull] IDialogCoordinator dialogCoordinator)
    {
        ...
        OpenSearchOptionsCommand = new RelayCommand(async () =>
        {
            ...
            {
                var content =
                    objectProvider.Get<SearchOptionsViewModel>(
        ...
        var searchInvoker =
            SearchHints.SearchRequested.Subscribe(request => searchInfoCollection.Add(request.Text, request.UseRegEx));
        ...
        var lineProxyFactory = new LineProxyFactory(textFormatter, lineMatches,
            horizonalScrollArgs.DistinctUntilChanged(), themeProvider);
        ...
        var inlineViewerVisible =
            isUserDefinedChanged.CombineLatest(showInline, (userDefined, showInlne) => userDefined && showInlne);
        ...
        var inline = searchInfoCollection.All.CombineLatest(inlineViewerVisible,
            (index, ud) => ud ? index : EmptyLineProvider.Instance);

        InlineViewer = inlineViewerFactory.Create(combinedSearchMetadataCollection, inline,
