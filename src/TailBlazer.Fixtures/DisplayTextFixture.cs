...
public class DisplayTextFixture
{
    ...
    public void CanVirtualise()
    {
        var input = new[]
            {
                new MatchedString("The cat "), new MatchedString("sat "), new MatchedString("on the mat"),
                new MatchedString(" and slept like a bat")
            }.Select(ms => new DisplayText(ms))
            .ToArray();


        var expected = new[] {new MatchedString("at "), new MatchedString("sat "), new MatchedString("on the m")}
            .Select(ms => new DisplayText(ms))
            .ToArray();
    ...
    public void CanVirtualise2()
    {
        ...
        var expected = new[] {new MatchedString("t "), new MatchedString("on the m")}
            .Select(ms => new DisplayText(ms))
            .ToArray();
