        ...
        var headSubscriber = tailSearch.Take(1).WithContinuation(() =>
            {
                ...
                return searchData.Connect(fss => fss.Segment.Type == FileSegmentType.Head)
                    ...
                    .WhereReasonsAre(ChangeReason.Add)
                    .SelectMany(changes =>
                        changes.Select(c => c.Current).OrderByDescending(c => c.Segment.Index).ToArray())
                    .ObserveOn(_scheduler)
                    .Synchronize(locker)
                    .Do(head => searchData.AddOrUpdate(new FileSegmentSearch(head.Segment,
                        FileSegmentSearchStatus.Searching)))
