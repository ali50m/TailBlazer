...
{
    public static IObservable<IChangeSet<T>> LogChanges<T>(this IObservable<IChangeSet<T>> source, ILogger logger,
        string label)
    {
    ...
    public static IObservable<T> LogValueAndErrors<T>([NotNull] this IObservable<T> source, [NotNull] ILogger logger,
    ...
    public static IObservable<IChangeSet<TObject, TKey>> LogChanges<TObject, TKey>(
        this IObservable<IChangeSet<TObject, TKey>> source, ILogger logger, string label)
    {
        if(!Debugger.IsAttached)
            return source;
